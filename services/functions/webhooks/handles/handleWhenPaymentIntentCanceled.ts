...
export const handleWhenPaymentIntentCanceled = async (transaction: Transactions) => {
  if (!transaction) {
    return
  }
  ...
  await Promise.all([
    transactionService.updateTransaction({
      ...
      data: {
        status: TransactionsStatus.CANCELED,
      },
      credentials: {
        isPublic: true,
      } as any
    ...
    transactionLogServices.createTransactionLog({
      data: {
        content: `Transaction ${transaction.transactionId} canceled`,
        transactionId: transaction.transactionId,
