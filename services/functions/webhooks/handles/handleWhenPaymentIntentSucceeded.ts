...
export const handleWhenPaymentIntentSucceeded = async (transaction: Transactions) => {
  if (!transaction) {
    return
  }
  ...
  await Promise.all([
    transactionService.updateTransaction({
      ...
      data: {
        status: TransactionsStatus.SUCCEED,
      },
      credentials: {
        isPublic: true,
      } as any
    ...
    transactionLogServices.createTransactionLog({
      data: {
        content: `Transaction ${transaction.transactionId} succeeded`,
        transactionId: transaction.transactionId,
